...
def handle_update_sensor_state(servent_id, state, attributes):
...
class ServEntSensor(RestoreSensor):
    ...
    def _update_servent_entity_config(self, config, device_config):
        ...
        # Sensor Attributes
        self._attr_device_class = toEnum(SensorDeviceClass, self.servent_config.get(
            SERVENT_DEVICE_CLASS, None))

        self._attr_native_unit_of_measurement = self.servent_config.get(
            SERVENT_UNIT_OF_MEASUREMENT, None)

        self._attr_state_class = self.servent_config.get(
            SERVENT_STATE_CLASS, None)

        self._attr_options = self.servent_config.get(
            SERVENT_ENUM_OPTIONS, None)
