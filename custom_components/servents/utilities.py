...
from .const import (
    ...
    SERVENT_DEVICE,
    SERVENT_DEVICE_ID,
    SERVENT_DEVICE_MANUFACTURER,
    SERVENT_DEVICE_MODEL,
    SERVENT_DEVICE_NAME,
    SERVENT_DEVICE_VERSION,
...
def stripNone(data):
    if isinstance(data, dict):
        return {
            k: stripNone(v) for k, v in data.items() if k is not None and v is not None
        }
    ...
    for value in servent_current_config[SERVENTS_CONFIG_ENTS].values():
        for conf in value.values():
            if SERVENT_DEVICE in conf:
                all_ids.append(f"device-{conf[SERVENT_DEVICE][SERVENT_DEVICE_ID]}")
    return [*set(all_ids)]
...
def load_config_from_file():
    ...
    config_from_file = {}
    try:
        config_from_file = yaml.load_yaml("servents.yaml")
...
def get_live_entities_from_cache(platform, servent_id):
    ...
    return (
        servent_live_entities[platform][servent_id]
        if servent_id in servent_live_entities[platform]
        else None
    )
...
def create_device_info(device_config):
    return (
        DeviceInfo(
            identifiers={(DOMAIN, f"device-{device_config[SERVENT_DEVICE_ID]}")},
            name=device_config[SERVENT_DEVICE_NAME],
            manufacturer=device_config.get(SERVENT_DEVICE_MANUFACTURER, "ServEnts"),
